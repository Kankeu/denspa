...
class VectorSearch:
    ...
    def __init__(self,folder_path=None,index_name=None,embedding_function=None,bm25_options={"k1":1.25,"b":.75}):
    ...
    @staticmethod
    def from_documents(documents,engine="*",lang=None,folder_path=None,index_name=None,embedding_function=None,bm25_options={"k1":1.25,"b":.75}):
    ...
    @staticmethod
    def load_local(folder_path,index_name,embedding_function=None):
    ...
    def similarity_search_with_score(self, query, k = 5, filter=None,lang=None,method="cascade"):
        if method=="faiss":
            ...
            def filter_func(metadata):
                return (filter is None or filter(metadata)) and any(all(metadata.get(k,None)==v for k,v in doc.metadata.items()) for doc,_ in results)
            tokenizer = Tokenizer(lang=lang)

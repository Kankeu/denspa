...
class BM25:
    ...
    def update(self):
        ...
        last_i = 0
        ntd = 0
        self.set_weights(scipy.sparse.lil_matrix(self.cvectors.shape,dtype=self.cvectors.dtype)) #csr_matrix
    ...
    def search(self,queries,k=5,filter=None):
        res = self.weights.dot(self.encode(queries).transpose())#cosine_similarity(self.weights,self.encode(queries))
        results = [[] for _ in range(len(queries))]
        for i in range(res.shape[0]):
            for j in range(res.shape[1]):
                if res[i,j]>0 and (filter is None or filter((i,res[i,j]))):
